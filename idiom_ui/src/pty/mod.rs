...
impl PtyShell {
    pub fn default_cmd(rect: Rect) -> anyhow::Result<Self> {
        Self::new(CommandBuilder::new_default_prog(), rect)
    }

    pub fn run(cmd: &str, rect: Rect) -> anyhow::Result<Self> {
        Self::new(CommandBuilder::new(cmd), rect)
    }

    pub fn new(mut cmd: CommandBuilder, rect: Rect) -> anyhow::Result<Self> {
    ...
    pub fn map_key(&mut self, key: &KeyEvent, backend: &mut impl Backend) -> std::io::Result<()> {
        self.select.clear();
    ...
    pub fn map_mouse(&mut self, event: MouseEvent, backend: &mut impl Backend) -> Message {
        match event {
            MouseEvent { kind: MouseEventKind::Down(MouseButton::Left), column, row, .. } => {
                let Some(Position { row, col }) = self.rect.relative_position(row, column) else {
                    self.select.clear();
            ...
            MouseEvent { kind: MouseEventKind::ScrollUp, column, row, .. } => {
                if !self.rect.contains_position(row, column) {
                    return Message::Skipped(event.kind);
                };
                self.select.clear();
                self.parser.scroll_up();
    ...
    pub fn fast_render(&mut self, backend: &mut impl Backend) {
    ...
    pub fn render(&mut self, backend: &mut impl Backend) {
        _ = self.parser.try_parse();
        self.inner_render(backend);
    }

    fn inner_render(&mut self, backend: &mut impl Backend) {
    ...
    fn render_no_select(&mut self, backend: &mut impl Backend) {
    ...
    fn render_with_select(&mut self, (from, to): (Position, Position), backend: &mut impl Backend) {
        let screen = self.parser.screen();
        let reset_style = backend.get_style();
        backend.reset_style();
        self.rect.clear(backend);
        {
            ...
            let Position { row: start_line, .. } = from;
            let Position { row: end_line, .. } = to;
            for line in self.rect.into_iter() {
                if let Some((index, text)) = text.next() {
                    if index < start_line as usize || index > end_line as usize {
                        backend.go_to(line.row, line.col);
                        _ = backend.write_all(&text);
                        backend.reset_style();
                        continue;
