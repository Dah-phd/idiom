...
pub trait Writable: Display {
    ...
    fn print(&self, backend: &mut impl Backend);
    /// prints bounded by line
    fn print_at(&self, line: Line, backend: &mut impl Backend);
    /// wraps within rect
    fn wrap(&self, lines: &mut impl IterLines, backend: &mut impl Backend);
    /// # Safety
    /// print truncated
    unsafe fn print_truncated(&self, width: usize, backend: &mut impl Backend);
    /// # Safety
    /// print truncated start
    unsafe fn print_truncated_start(&self, width: usize, backend: &mut impl Backend);
...
impl Text {
    ...
    #[inline]
    pub fn simple_wrap(&self, lines: &mut RectIter, backend: &mut impl Backend) {
    ...
    #[inline]
    fn wrap_with_remainder(&self, lines: &mut impl IterLines, backend: &mut impl Backend) -> Option<usize> {
        if self.is_simple() {
    ...
    #[inline]
    pub fn wrap_with_remainder_simple(&self, lines: &mut impl IterLines, backend: &mut impl Backend) -> Option<usize> {
    ...
    #[inline]
    pub fn wrap_with_remainder_complex(&self, lines: &mut impl IterLines, backend: &mut impl Backend) -> Option<usize> {
...
impl Writable for StyledLine {
    ...
    unsafe fn print_truncated(&self, mut width: usize, backend: &mut impl Backend) {
        for text in self.inner.iter() {
            if text.width > width {
