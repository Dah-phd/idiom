...
impl Rect {
    ...
    pub fn relative_position(&self, row: u16, column: u16) -> Option<Position> {
        match self.contains_position(row, column) {
            true => Some(Position { row: (row - self.row), col: (column - self.col) }),
            false => None,
        }
    }
    ...
    #[inline]
    pub fn border_title_styled(&self, text: &str, style: ContentStyle, backend: &mut impl Backend) {
        if self.borders.contains(Borders::TOP) {
    ...
    #[inline]
    pub fn border_title_bot_styled(&self, text: &str, style: ContentStyle, backend: &mut impl Backend) {
        if self.borders.contains(Borders::BOTTOM) {
            backend.print_styled_at(self.row + self.height + 1, self.col, text.truncate_width(self.width).1, style)
        }
    }

    pub fn draw_borders(&self, set: Option<BorderSet>, fg: Option<Color>, backend: &mut impl Backend) {
