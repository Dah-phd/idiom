...
impl Line {
    ...
    #[inline]
    pub fn fill_styled(self, symbol: char, style: ContentStyle, backend: &mut impl Backend) {
        let text = (0..self.width).map(|_| symbol).collect::<String>();
    ...
    #[inline]
    pub fn render_centered_styled(self, text: &str, style: ContentStyle, backend: &mut impl Backend) {
        let (remaining_width, text) = text.truncate_width(self.width);
    ...
    #[inline]
    pub fn render_left_styled(self, text: &str, style: ContentStyle, backend: &mut impl Backend) {
        let (pad_width, text) = text.truncate_width_start(self.width);
        backend.go_to(self.row, self.col);
        if pad_width != 0 {
            backend.pad(pad_width);
    ...
    #[inline]
    pub fn render_styled(self, text: &str, style: ContentStyle, backend: &mut impl Backend) {
        let Line { width, row, col } = self;
        let (pad_width, text) = text.truncate_width(width);
...
pub struct LineBuilder<'a, T: Backend> {
    row: u16,
    col: u16,
    remaining: usize,
    backend: &'a mut T,
...
impl<T: Backend> LineBuilder<'_, T> {
...
pub struct LineBuilderRev<'a, T: Backend> {
    row: u16,
    col: u16,
    remaining: usize,
    backend: &'a mut T,
...
impl<T: Backend> LineBuilderRev<'_, T> {
    /// returns Ok(bool) -> if true line is not full, false the line is finished
    pub fn push(&mut self, text: &str) -> bool {
