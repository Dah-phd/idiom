...
impl Editor {
    ...
    pub fn select_scope(&mut self) {
        ...
        let select = match self.file_type.scope_type() {
            ...
            ScopeType::Indent => select_indent(self),
            ScopeType::Marked { open: opening, close: closing } => select_between_chars(self, opening, closing),
        };
        match select {
            Some((from, to)) => self.cursor.select_set(from, to),
