use super::action_buffer::ActionBuffer;
pub use super::edits::{Edit, EditMetaData};
use crate::{
            ...
            Some((mut from, mut to)) => {
                ...
                match get_closing_char(ch) {
                    Some(closing) => {
                        ...
                        let second_edit = Edit::record_in_line_insertion(from, ch.to_string());
                        from.char += 1;
                        if from.line == to.line {
                            to.char += 1;
                        }
...
impl EditType {
    pub fn apply_rev(&self, content: &mut Vec<impl EditorLine>) -> (CursorPosition, Option<Select>) {
        match self {
    ...
    pub fn apply(&self, content: &mut Vec<impl EditorLine>) -> (CursorPosition, Option<Select>) {
        match self {
