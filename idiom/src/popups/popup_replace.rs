...
impl ReplacePopup {
    pub fn run_inplace(gs: &mut GlobalState, workspace: &mut Workspace, tree: &mut Tree, term: &mut EditorTerminal) {
        ...
        let mut popup: ReplacePopup = Self {
            ...
            new_text: TextField::new(String::new()),
            state: usize::default(),
        };
...
impl Popup for ReplacePopup {
    fn map_keyboard(&mut self, key: KeyEvent, components: &mut Components) -> Status {
        ...
        match key.code {
            ...
            _ => {
                let field_status = match self.on_text {
                    true => self.new_text.map(&key),
                    false => self.pattern.map(&key),
                };
                ...
                match field_status {
                    ...
                    FieldStatus::PasteInvoked => {
                        if gs
                            .clipboard
                            .pull()
                            .map(|clip| self.pattern.paste_passthrough(clip).is_updated())
                            .unwrap_or_default()
                        {
                            self.options.clear();
                            editor.find(&self.pattern.text, &mut self.options);
                    ...
                    FieldStatus::NotMapped | FieldStatus::Skipped => (),
                }
            }
        }
        Status::Pending
