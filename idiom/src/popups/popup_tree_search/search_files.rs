...
use idiom_ui::{
    ...
    state::State,
    text_field::{Status as FieldStatus, TextField},
};
...
impl Popup for ActiveFileSearch {
    ...
    fn map_keyboard(&mut self, key: KeyEvent, components: &mut Components) -> Status {
        ...
        match self.pattern.map(&key) {
            FieldStatus::Copy(clip) => {
                gs.clipboard.push(clip);
                return Status::Pending;
            }
            FieldStatus::Cut(clip) => {
                gs.clipboard.push(clip);
                self.force_render(gs);
                return Status::Pending;
            }
            FieldStatus::PasteInvoked => {
                if gs.clipboard.pull().map(|clip| self.pattern.paste_passthrough(clip).is_updated()).unwrap_or_default()
                {
                    self.force_render(gs);
                }
                return Status::Pending;
            }
            FieldStatus::Updated | FieldStatus::UpdatedCursor => {
                self.force_render(gs);
                return Status::Pending;
            }
            FieldStatus::Skipped => {
    ...
    fn paste_passthrough(&mut self, clip: String, _: &mut Components) -> bool {
        if self.pattern.paste_passthrough(clip).is_text_updated() {
            _ = self.collect_data();
            return true;
        }
        false
...
fn build_path_line<T: Backend>((path, ..): &SearchResult, mut builder: LineBuilder<T>) {
    builder.push(&format!("{}", path.display()));
}

fn build_text_line<T: Backend>((.., line_txt, line_idx): &SearchResult, mut builder: LineBuilder<T>) {
