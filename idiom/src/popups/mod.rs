...
pub fn get_new_screen_size(backend: &mut impl Backend) -> IdiomResult<(Width, Height)> {
    loop {
...
pub fn get_init_screen(backend: &mut impl Backend) -> IdiomResult<Rect> {
...
pub fn checked_new_screen_size(width: Width, height: Height, backend: &mut impl Backend) -> (Width, Height) {
