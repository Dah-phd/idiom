...
#[inline(always)]
pub fn cursor(code: &mut EditorLine, ctx: &mut LineContext, line: Line, backend: &mut impl Backend) {
...
#[inline(always)]
fn render_no_select(code: &mut EditorLine, line_width: usize, ctx: &mut LineContext, backend: &mut impl Backend) {
    if code.is_simple() {
...
#[inline(always)]
pub fn cursor_fast(code: &mut EditorLine, ctx: &mut LineContext, line: Line, backend: &mut impl Backend) {
    let select = ctx.get_select(line.width);
