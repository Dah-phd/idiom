...
pub fn basic(line: &EditorLine, ctx: &LineContext, backend: &mut impl Backend) {
...
pub fn select(line: &EditorLine, ctx: &LineContext, select: Range<usize>, backend: &mut impl Backend) {
    let char_position = ctx.lexer.char_lsp_pos;
...
pub fn partial(code: &mut EditorLine, ctx: &mut LineContext, mut line_width: usize, backend: &mut impl Backend) {
    let cursor_idx = ctx.cursor_char();
    let char_position = ctx.lexer.char_lsp_pos;
