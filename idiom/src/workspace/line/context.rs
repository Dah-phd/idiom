...
use idiom_ui::{backend::Backend, layout::Line};
...
impl<'a> LineContext<'a> {
    ...
    #[inline]
    pub fn setup_cursor(&mut self, line: Line, backend: &mut impl Backend) -> usize {
        self.line_number += 1;
    ...
    #[inline]
    pub fn setup_line(&mut self, line: Line, backend: &mut impl Backend) -> usize {
        self.line_number += 1;
        let text = format!("{: >1$} ", self.line_number, self.line_number_offset);
        let remaining_width = line.width - text.len();
    ...
    #[inline]
    pub fn wrap_line(&mut self, line: Line, backend: &mut impl Backend) {
