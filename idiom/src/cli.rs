...
use idiom_ui::{backend::Backend, layout::Rect, state::State};
...
impl Args {
    pub fn collect(self, backend: &mut impl Backend) -> IdiomResult<Option<PathBuf>> {
        match self.path {
            Some(rel_path) => {
...
impl TreeSeletor {
    ...
    pub fn select(backend: &mut impl Backend, path: PathBuf) -> IdiomResult<Option<PathBuf>> {
        let tree_selector = Self::new(path)?;
        tree_selector.run(backend)
    }

    pub fn select_home(backend: &mut impl Backend) -> IdiomResult<Option<PathBuf>> {
    ...
    fn run(mut self, backend: &mut impl Backend) -> IdiomResult<Option<PathBuf>> {
    ...
    pub fn render_stateless(&mut self, rect: Rect, backend: &mut impl Backend) {
    ...
    pub fn fast_render_stateless(&mut self, rect: Rect, backend: &mut impl Backend) {
        if self.rebuild {
            self.rebuild = false;
            self.render_stateless(rect, backend);
        };
