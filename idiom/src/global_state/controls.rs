...
impl Mode {
    #[inline]
    pub fn render(&self, line: Line, backend: &mut impl Backend) {
        match self {
    ...
    #[inline]
    pub fn render_select_mode(line: Line, backend: &mut impl Backend) {
    ...
    #[inline]
    pub fn render_insert_mode(line: Line, backend: &mut impl Backend) {
        let mut style = ContentStyle::reversed();
        style.add_bold();
    ...
    match event.kind {
        ...
        MouseEventKind::Down(MouseButton::Left) => {
            ...
            if let Some(position) = gs.tree_area.relative_position(event.row, event.column) {
                if let Some(path) = tree.mouse_select(position.row as usize + 1, gs) {
            ...
            if let Some(pos) = gs.tab_area.relative_position(event.row, event.column) {
                if !ws.is_empty() {
                    gs.insert_mode();
                    if let Some(idx) = ws.select_tab_mouse(pos.col as usize) {
        ...
        MouseEventKind::Down(MouseButton::Right) => {
            if let Some(position) = gs.tab_area.relative_position(event.row, event.column) {
                if !ws.is_empty() {
                    gs.insert_mode();
                    if let Some(idx) = ws.select_tab_mouse(position.col as usize) {
                        ws.activate_editor(idx, gs);
                        ws.close_active(gs);
                    }
                }
            }
            if let Some(position) = gs.editor_area.relative_position(event.row, event.column).map(Into::into) {
                if let Some(editor) = ws.get_active() {
